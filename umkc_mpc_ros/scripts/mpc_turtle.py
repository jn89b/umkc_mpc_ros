...
from umkc_mpc_ros import TurtleBotNode, quaternion_tools
...
class MPCTurtleNode(TurtleBotNode.TurtleBotNode):
    def __init__(self, node_name, namespace =''):
        ...
        self.target_subscriber = self.create_subscription(
            Odometry, "turtle/odom", self.target_odom_cb, 10)
        
        self.target_position = [None, None]
        self.target_psi = None


    def target_odom_cb(self,msg:Odometry):
        """subscribe to odometry"""
        self.target_position[0] = msg.pose.pose.position.x
        self.target_position[1] = msg.pose.pose.position.y
...
def main_mpc():
    ...
    while rclpy.ok():
        ...
        end_pos = mpc_turtle_node.target_position
        
        if end_pos == [None,None]:
            rclpy.spin_once(mpc_turtle_node)
            continue

        print("error", compute_error(curr_pos, end_pos))        

        if compute_error(curr_pos, end_pos) <= 0.15:
            print("reached goal")
            mpc_turtle_node.move_turtle(0.0, 0.0)
        ...
        mpc_turtle_node.move_turtle(cmd_vel, ang_vel_cmd)
